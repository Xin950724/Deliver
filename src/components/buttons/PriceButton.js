...
import MoneyReciveImage from '../../assets/images/user/vehicle/money-recive.svg'
import MoneySendImage from '../../assets/images/user/vehicle/money-send.svg'

const { width } = Dimensions.get('window')
const scaleFactor = width / 414

const PriceButton = ({ selected, value }) => {
    ...
    return (
        <TouchableOpacity style={select == true ? styles.selected_button : styles.button} onPress={() => setSelected(!select)}>
            <View style={styles.calendar_image}>
                {select == true ? <MoneyReciveImage /> : <MoneySendImage />}
            </View>
